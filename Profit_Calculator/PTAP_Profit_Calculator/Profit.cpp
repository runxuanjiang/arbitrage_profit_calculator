...
public:
	...
	std::vector<std::pair <float, float>> pair_1; // USD to BTC, we only need to look at the ask price
	std::vector<std::pair <float, float>> pair_2; // ETH to BTC, we only need to look at the bid price
	std::vector<std::pair <float, float>> pair_3; // ETH to USD, we only need to look at the bid price
	float time;
...
float profit_calc(Timestamp &time) {
	...
	if (total_invest > 0) return total_profit / total_invest;
	else return 0;
}
...
// *********************************************************************************************************************************************************************
int main() {
	std::ifstream ask_one("BTC_USD_asks.txt");
	std::ifstream bid_two("ETH_BTC_bids.txt");
	std::ifstream bid_three("ETH_USD_bids.txt");
