...
class Orderbook:
    ...
    def show_head_bid(self, n = 50):
        ...
        for i in range(n):
            ...
            order = self.order_dict[id]
            print order.get_price(), order.get_remaining()
    def show_head_gap(self):
        # print the first n bids and first n asks
        # Input:
        #       n - optional, number of orders in each list to be printed
        # Returns:
        # Modifies:
            idask = self.ask_list[0]
            orderask = self.order_dict[idask]
            topask = orderask.get_price()
            idbid = self.bid_list[0]
            orderbid = self.order_dict[idbid]
            topbid = orderbid.get_price()
            gap = topask - topbid
            print gap
